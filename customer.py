...
class Cust_win:
    def __init__(self,root):
        ...
        Table_Frame=LabelFrame(self.root,bd=2,relief=RIDGE,text="View Deatils And Search System",font=("arial",12,"bold"),padx=2,)
    ...
    def update(self):
        if self.var_mobile.get()=="":
            messagebox.showerror("Error","Plaease enter moblie number",parent=self.root)
        else:
            conn=mysql.connector.connect(host="localhost",username="root",password="123456789",database="management")
            my_cursor=conn.cursor()
            my_cursor.execute("update customer set Name=%s,mother=%s,Gender=%s,PostCode=%s,Moblie=%s,Email=%s,Nationality=%s,Idproof=%s,Idnumber=%s,Address=%s where Ref=%s",(
